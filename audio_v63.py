...
class JingleDetector:
    def __init__(self, audio_file_path, jingle_start_time, jingle_end_time, threshold=0.7, min_segment_length=30):
        """
        Initialize the jingle detector by extracting template directly from source file.

        Args:
            audio_file_path: Path to the source audio file (same file we'll search)
            jingle_start_time: Start time of jingle template in seconds
            jingle_end_time: End time of jingle template in seconds
            threshold: Correlation threshold for detection (0-1, higher = stricter)
            min_segment_length: Minimum length of content segments in seconds
        """
        self.threshold = threshold
        self.min_segment_length = min_segment_length
        self.source_file = audio_file_path

        print(f"Extracting jingle template from source file...")
        print(f"Template range: {jingle_start_time:.1f}s to {jingle_end_time:.1f}s")

        # Load the source audio file and extract the jingle template
        audio, sr = librosa.load(audio_file_path, sr=22050)

        # Extract the jingle template from specified time range
        start_sample = round(jingle_start_time * sr)
        end_sample = round(jingle_end_time * sr)

        if end_sample > len(audio):
            raise ValueError(
                f"Jingle end time ({jingle_end_time:.1f}s) exceeds audio duration ({len(audio) / sr:.1f}s)")

        self.template = audio[start_sample:end_sample]
        self.template_sr = sr
        ...
        print(f"Template extracted: {template_duration:.2f}s ({len(self.template)} samples)")
        ...
        # Test template against its source location
        print(f"Testing template against source location...")
        source_correlation = self._test_template_vs_source(jingle_start_time)
        print(f"Template vs source: {source_correlation:.6f}")

        if source_correlation > 0.99:
            print("✓ PERFECT template extraction!")
        elif source_correlation > 0.95:
            print("✓ Excellent template extraction")
        else:
            print(f"⚠ Warning: Template extraction may have issues (correlation: {source_correlation:.3f})")

        # Save the extracted template for reuse
        self._save_template("jingle_template.wav")

    def _test_template_vs_source(self, template_start_time):
        """Test if extracted template matches its source location."""
        audio, sr = librosa.load(self.source_file, sr=22050)
        template_len = len(self.template)

        start_sample = round(template_start_time * sr)
        end_sample = start_sample + template_len

        source_segment = audio[start_sample:end_sample]

        # Make sure they're the same length
        min_len = min(len(source_segment), len(self.template))
        source_segment = source_segment[:min_len]
        template_segment = self.template[:min_len]

        correlation = np.corrcoef(source_segment.flatten(), template_segment.flatten())[0, 1]
        if np.isnan(correlation):
            correlation = 0

        return correlation

    def _save_template(self, output_path):
        """Save the extracted template for reuse with other files."""
        try:
            sf.write(output_path, self.template, self.template_sr)
            print(f"✓ Template saved to: {output_path}")
            print(f"  You can reuse this template for other audio files!")
        except Exception as e:
            print(f"⚠ Failed to save template: {e}")

    @classmethod
    def from_template_file(cls, template_path, threshold=0.7, min_segment_length=30):
        """
        Create a JingleDetector using a saved template file.
        ...
        detector.min_segment_length = min_segment_length
        detector.source_file = None

        # Load the template
        detector.template, detector.template_sr = librosa.load(template_path, sr=22050)

        # Normalize template
        detector.template = detector.template / np.max(np.abs(detector.template))

        template_duration = len(detector.template) / detector.template_sr
        print(f"Template loaded: {template_duration:.2f}s ({len(detector.template)} samples)")

        # Create multiple representations for robust matching
        detector.template_mfcc = librosa.feature.mfcc(y=detector.template, sr=detector.template_sr, n_mfcc=13)
        detector.template_chroma = librosa.feature.chroma_stft(y=detector.template, sr=detector.template_sr)

        # Test self-correlation
        self_corr = np.corrcoef(detector.template.flatten(), detector.template.flatten())[0, 1]
        print(f"Template self-correlation: {self_corr:.3f} (should be 1.0)")
    ...
    def detect_jingles(self, audio_file_path, method='combined'):
        """
        ...
        if method == 'correlation' or method == 'combined':
            correlations = self._cross_correlation_detection(audio, sr)

        if method == 'mfcc' or method == 'combined':
            mfcc_matches = self._mfcc_detection(audio, sr)

        if method == 'chroma' or method == 'combined':
            chroma_matches = self._chroma_detection(audio, sr)

        # Combine results based on method
        if method == 'correlation':
            candidates = correlations
        elif method == 'mfcc':
            candidates = mfcc_matches
        elif method == 'chroma':
            candidates = chroma_matches
        else:  # combined
            # Use weighted combination of all methods
            candidates = self._combine_detections(correlations, mfcc_matches, chroma_matches)
    ...
    def _filter_detections(self, candidates, audio, sr):
        ...
        print(f"DEBUG: Filtering {len(candidates)} candidates")

        # First, filter by a reasonable score threshold
        ...
        print(f"DEBUG: {len(high_score_candidates)} candidates above score threshold {score_threshold}")

        if not high_score_candidates:
            print("DEBUG: No high-scoring candidates found, lowering threshold to 0.6")
            score_threshold = 0.6
            high_score_candidates = [(time, score) for time, score in candidates if score >= score_threshold]
            print(f"DEBUG: {len(high_score_candidates)} candidates above score threshold {score_threshold}")
        ...
        print("DEBUG: Top 10 high-scoring candidates:")
        for i, (time_pos, score) in enumerate(high_score_candidates[:10]):
        ...
        for time_pos, score in high_score_candidates:
            ...
            if not too_close:
                ...
                print(f"DEBUG: Added {time_pos:.1f}s (score {score:.3f})")
            else:
                print(f"DEBUG: Rejected {time_pos:.1f}s (too close to existing detection)")

            # Limit to reasonable number of detections
            if len(filtered) >= 10:
                print("DEBUG: Reached maximum 10 detections, stopping")
                break

        print(f"DEBUG: Final filtered detections: {len(filtered)}")
        return filtered
...
def main():
    ...
    print("RUNNING VERSION 58 - ADDED PROGRESS TIMESTAMPS")
    print("=" * 60)
    print("DEBUG: Main function started successfully")

    # Configuration
    audio_file_path = "file_92min.m4a"  # Your M4A file
    ...
    try:
        ...
        detector = JingleDetector(
            ...
            jingle_start_time=jingle_start_time,
            jingle_end_time=jingle_end_time,
