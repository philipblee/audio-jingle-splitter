...
class JingleDetector:
    ...
    def split_audio_by_jingles(self, audio_file_path: str, jingle_times: List[float],
                               output_dir: str = "segments", jingle_duration: float = 8.0) -> None:
        """
        ...
        if self._has_ffmpeg():
            self._split_with_ffmpeg(audio_file_path, jingle_times, output_dir, jingle_duration)
        else:
            self._split_with_librosa(audio_file_path, jingle_times, output_dir, jingle_duration)
    ...
    def _split_with_ffmpeg(self, audio_file_path: str, jingle_times: List[float],
                           output_dir: str, jingle_duration: float) -> None:
        ...
        for i, (start_time, end_time) in enumerate(segments):
            ...
            if duration < self.min_segment_length:
                print(f"Skipping segment {i + 1}: too short ({duration:.1f}s)")
                continue

            output_file = Path(output_dir) / f"{base_name}_segment_{i + 1:02d}.mp3"
            ...
            try:
                ...
                print(f"Failed to extract segment {i + 1}: {e.stderr}")
    ...
    def _split_with_librosa(self, audio_file_path: str, jingle_times: List[float],
        ...
        try:
            ...
            for i, (start_time, end_time) in enumerate(segments):
                ...
                # Save as WAV
                output_file = Path(output_dir) / f"{base_name}_segment_{i + 1:02d}.wav"
...
def process_audio_file(audio_file_path: str, template_path: str,
                       ...
                       threshold: float = 0.5,
                       min_segment_length: int = 1500) -> bool:
    """
...
def main():
    ...
    # Configuration
    audio_file_path = "big_history1.m4b"
